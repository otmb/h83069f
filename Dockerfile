...
RUN buildDeps='minicom' \
  ...
  && cd /usr/src \
  && curl -SL "ftp://core.ring.gr.jp/pub/GNU/binutils/binutils-2.19.1.tar.bz2" -o binutils.tar.bz2 \
  && curl -SL "ftp://core.ring.gr.jp/pub/GNU/gcc/gcc-3.4.6/gcc-3.4.6.tar.bz2" -o gcc.tar.bz2 \
